...
export class DefaultUIService {
    ...
    private menuItems:Array<HTMLDivElement|null> = [];
    private menuOpen = true;

    constructor(view:ViewService, session:SessionService) {
        const config = session.configuration.defaultUI || {};
        ...
        if (document && !config.disable) {
            ...
            this.addMenuItem(vrIcon, 'Toggle VR', ()=>{

            });
            this.addMenuItem(eyeIcon, 'Select Reality Viewer...', ()=>{
    ...
    private addMenuItem(icon?:string, hint?:string, onSelect?:()=>any) {
